	...
	if ($res_query_user->num_rows > 0) {
		...
		if ($res_active_code->num_rows > 0) {
			...
			if($active_code > $to_amount){
				// 修改当前用户的激活码数量
				mysqli_query($conn,"update user set activecode=activecode-'$to_amount' where uid='$uid'");
				...
				if ($affectRow == 0 || mysqli_errno($conn)) {  
					...
					if ($affectRow == 0 || mysqli_errno($conn)) {
						...
						// 记录转账
						$opremarks = "转".$to_amount."个激活码给：" .$uname;
						$opsql = "insert into oprecord(uid, optype, opedid, opremarks) values('$uid','3', '$uname', '$opremarks')";
						$conn->query($opsql);
						$affectRow = mysqli_affected_rows($conn);
						if ($affectRow == 0 || mysqli_errno($conn)) {  
						    // 回滚事务重新提交  
